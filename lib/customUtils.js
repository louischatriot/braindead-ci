...
function ensureDirectoryExists (dir, cb) {
  ...
  fs.exists(currentPart, function (exists) {   // WTF, this call to fs.exists creates the directory, that doesn't make any sense
 ...
 * @param {Funtion} callback Signature: err, { salt, derivedKey }
 */
function encryptPassword (password, callback) {
  var randomSalt = uid(12);

  crypto.pbkdf2(password, randomSalt, 10000, 24, function (err, derivedKey) {
    if (err) { return callback(err); }

    return callback(null, { salt: randomSalt, derivedKey: derivedKey });
  });
}
...
function checkPassword (password, encryptedPassword, callback) {
  if (!encryptedPassword.salt || !encryptedPassword.derivedKey) { return callback("encryptedPassword doesn't have the right format"); }

  crypto.pbkdf2(password, encryptedPassword.salt, 10000, 24, function (err, derivedKey) {
    if (err) { return callback(err); }
