var db = require('./db')
  ...
  , hasher = new NodePbkdf2(config.passwordEncryption)
  ;
...
/**
 * Create a user
 * @param {Object} userData (login, password)
...
User.createUser = function (userData, cb) {
  ...
  db.users.findOne({ login: userData.login }, function (err, user) {
    if (err) { return callback(err); }
    if (user) { return callback({ validationErrors: { login: 'This login is already taken' } }); }

    hasher.encryptPassword(userData.password, function (err, encryptedPassword) {
      if (err) { return callback(err); }
...
User.checkCredentials = function (login, password, callback) {
  db.users.findOne({ login: login }, function (err, user) {
    ...
    hasher.checkPassword(password, user.password, function (err, ok) {
      if (err) { return callback(err); }
...
User.changePassword = function (login, password, newPassword, cb) {
  ...
  db.users.findOne({ login: login }, function (err, user) {
    ...
    hasher.checkPassword(password, user.password, function (err, ok) {
      ...
      hasher.encryptPassword(newPassword, function (err, encryptedPassword) {
        if (err) { return callback(err); }
