...
Job.prototype.build = function (out, callback) {
  var self = this
    ...
    , buildAndTest = self.repoSSHUrl && self.repoSSHUrl.length > 0 &&   // If no branch or repoSSHUrl is provided, that means the user doesn't want
                     self.branch && self.branch.length > 0              // to use the CI capability but just execute the deploy script
    , buildSteps = []
    ;
  ...
  customUtils.ensureDirectoryExists(Job.getBuildsDir(this.name), function (err) {
    ...
    if (buildAndTest) {
      // Pulling repo
      buildSteps.push(self.pullRepo.bind(self, channel));

      // Checking dependencies
      buildSteps.push( function (cb) {
      ...
      buildSteps.push(function (cb) {
        ...
        self.reinstallDependencies(channel, function (err) { return cb(err); });
      });

      // Running tests
      buildSteps.push(function (cb) {
        channel.write("=== Running test script ===\n");
        var script = "cd " + Job.getRepoPath(self.name).replace(/ /g, '\\ ') + ";" + self.testScript;
        customUtils.executeBashScript(script, null, channel, cb);
      });
    }

    // Deployment
    // This is the only step that will be built no matter what
    // In that case Braindead is used as a task manager
    buildSteps.push(function (cb) {
      channel.write("=== Running deployment script ===\n");
      customUtils.executeBashScript(self.deployScript, { REINSTALL_DEPS: needToReinstallDeps }, channel, cb);
    });

    if (buildAndTest) {
      // Copying new dependencies
      buildSteps.push(self.copyDependenciesFiles.bind(self));
    }

    async.waterfall(buildSteps, function (err) {
