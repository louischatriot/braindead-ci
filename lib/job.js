...
Job.prototype.save = function (callback) {
  ...
  db.jobs.findOne({ name: this.name }, function (err, job) {
    if (err) { return callback(err); }
    if (job) {
      db.jobs.update({ name: this.name }, toBeSerialized, { multi: false }, function (err) { return callback(err); });
    } else {
      db.jobs.insert(toBeSerialized, callback);
    }
  });
};
