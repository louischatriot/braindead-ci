...
Job.validate = function (jobData) {
  var validators = {}
    , errors = []
    , fields = Object.keys(jobData), field, i
    ;
  ...
  registerValidator('name', validation.validateJobName, 'The name must be composed of between 1 and 20 alphanumerical characters and spaces');
...
Job.prototype.setEnabledValue = function (newValue, callback) {
  ...
  self.save(function (err) {
    if (err) { return callback(err); }
    self.addJobMetadata(self.name, callback);
  });
};
...
Job.prototype.copyDependenciesFiles = function (callback) {   // Copy dependency files to remember them next time we check if need to reinstall
  customUtils.ensureFolderExists(Job.getDependenciesInfoDir(this.name), function (err) {
    if (err) { return callback(err); }
    customUtils.copySafe(Job.getRepoPath(this.name) + '/npm-shrinkwrap.json', Job.getDependenciesInfoDir(this.name) + '/npm-shrinkwrap.json', function (err) {
      if (err) { return callback(err); }
      customUtils.copySafe(Job.getRepoPath(this.name) + '/package.json', Job.getDependenciesInfoDir(this.name) + '/package.json', function (err) {
        return callback(err);
      });
    });
  });
