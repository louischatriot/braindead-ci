 ...
 */
Job.getEditableProperties = function () { return ['name', 'githubRepoUrl' , 'repoSSHUrl' , 'branch']; };
...
Job._edit.githubRepoUrl = function (newValue, callback) { return callback(); }

Job._edit.repoSSHUrl = function (newValue, callback) { return callback(); }

Job._edit.branch = function (newValue, callback) { return callback(); }
...
Job.prototype.edit = function (newOptions, callback) {
  ...
  , function (cb) {
    // For all modified editable properties, apply the side effects
    var property = editableProperties[i];
    i += 1;
    if (self[property] !== newOptions[property]) {
      console.log('-- CHANGE --');
      return Job._edit[property].call(self, newOptions.name, cb);
    } else {
      return cb();
    }
  }
