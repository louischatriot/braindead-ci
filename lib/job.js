...
Job.prototype.reinstallDependencies = function (outs, callback) {
  ...
  //childProcess.exec('ls', { cwd: Job.getRepoPath(self.name) }, function (err, stdout, stderr) {
    ...
    customUtils.pipeStreamIntoMultipleStreams(installer.stderr, outs);
...
Job.prototype.build = function (out, callback) {
  ...
  customUtils.ensureFolderExists(Job.getBuildsDir(this.name), function (err) {
    ...
    buildReport.on('error', function (e) {
      console.log("=========== THE FUCKING ERROR ======");
      console.log(e);
    });
    ...
    //}
      ...
      self.reinstallDependencies([out, buildReport, process.stdout], function (err) {
        if (err) { buildReport.write("Couldn't pull the repository, error given: ", err); }
        cb(err);
      });
    }
    , function (cb) {
      ...
      //setTimeout(bloup, 100);
      self.runTests([out, buildReport, process.stdout], function (err) {
