...
function Job (jobData) {
  ...
  // Give the job its build step functions
  Object.keys(basicSteps).forEach(function (step) {
    self[step] = basicSteps[step];
  });
}
...
Job.prototype.build = function (out, callback) {
  ...
  self.buildingSandbox = {};   // Usable by the build steps to 
  ...
  customUtils.ensureDirectoryExists(Job.getBuildsDir(this.name), function (err) {
    ...
    if (buildAndTest) {
      ...
      buildSteps.push(self.checkIfDependenciesNeedToBeReinstalled.bind(self, channel));
      buildSteps.push(self.reinstallDependencies.bind(self, channel));
      buildSteps.push(self.runTestScript.bind(self, channel));
    }

    buildSteps.push(self.runDeployScript.bind(self, channel));

    if (buildAndTest) {
      buildSteps.push(self.rememberDependencies.bind(self));
    }
