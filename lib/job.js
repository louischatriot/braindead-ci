...
function Job (jobData) {
  ...
  // TODO: This only useful for retrocompatibility and will be removed
...
Job.removeJob = function (name, cb) {
  ...
  db.jobs.findOne({ name: name }, function (err, job) {
    if (err) { return callback(err); }
    if (!job) { returncallback("Job doesn't exist"); }
...
Job.prototype.advertiseOnHipchat = function (buildSuccessful) {
  ...
  db.settings.findOne({ type: 'generalSettings' }, function (err, settings) {
    ...
    buildUrl = url.resolve(settings.braindeadRootUrl, buildUrl);
