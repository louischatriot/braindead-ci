...
Job.prototype.advertiseOnHipchat = function (buildSuccessful) {
  ...
  if (buildSuccessful) {
    messageToSend.message = "(fuckyeah) Build and deploy successful for job " + this.name;
  } else {
    messageToSend.message = "(boloss) Build and deploy failed for job " + this.name;
  }
  ...
  customUtils.ensureFolderExists(Job.getBuildsDir(this.name), function (err) {
    ...
    , function (cb) {
      ...
      self.checkIfDependenciesNeedToBeReinstalled(channel, function (err, need) {
        if (err) { channel.write("=== Problem while checking if we need to reinstall dependencies, error given: " + err); return cb(err); }
