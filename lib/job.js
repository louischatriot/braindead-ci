...
//childProcess.exec('git status', { cwd: '/home/louis' }
  ...
  async.waterfall([
    function (cb) {   // Ensure the repo exists, create it if needed
      customUtils.ensureFolderExists(Job.getRepoPath(self.name), function (err) {
        ...
        childProcess.exec('git status', { cwd: Job.getRepoPath(self.name) }, function (err, stdout, stderr) {
          console.log("================");
          console.log(err);
          console.log('---');
          console.log(stdout);
          ...
          childProcess.exec('git clone ' + self.repo + ' ' + self.name, {  cwd: Job.getRepoPath(self.name)  }, function (err, stdout, stderr) {
          console.log(err);
          console.log('---');
          console.log(stdout);
            return cb(err);
