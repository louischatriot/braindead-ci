...
function validateJobGithubRepoUrl (value) {
...
function validateJobRepoSSHUrl (value) {
  if (value && value.length > 0) {
    return true;
  } else {
    return false;
...
registerValidator('job', 'githubRepoUrl', 'Please enter a valid Git repo SSH url', validateJobGithubRepoUrl);
registerValidator('job', 'repoSSHUrl', 'Please enter a valid Git repo SSH url', validateJobRepoSSHUrl);
registerValidator('job', 'branch', 'Please enter a branch name', validateJobBranch);
registerValidator('job', 'testScript', 'Please enter a deploy script', accept);
registerValidator('job', 'deployScript', 'Please enter a deploy script', accept);
